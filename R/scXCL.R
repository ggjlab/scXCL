...
#' Xenopus Cell Landscape mapping
#' description
...
scXCL <- function(scdata,numbers_plot=3){
  ...
  result[['scXCL']]<-scblast.result
  result[['scXCL_probility']]<-cors_out
  return(result)
}
...
plotXCL <- function(XCL_result,interactive_plot=F, numbers_plot=3, col_font_size = 1, row_font_size=8, show_col=T,show_bar=T, show_tree = T){
  ...
  colnames(cors_out)<- c("Cell","Cell type","Score")
  cors_out <- as.data.frame(cors_out %>% group_by(Cell) %>% top_n(n=numbers_plot,wt=Score))
  XCL_result$scXCL_probility <- cors_out
  XCL_result$top_cors <- numbers_plot
